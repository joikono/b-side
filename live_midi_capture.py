# # live_midi_capture.py - Real-time MIDI input capture and analysis with WebSocket streaming
...
#     NOW INCLUDES: Real-time WebSocket streaming for live piano sound.
#     """
...
#         self.streaming_queue = queue.Queue(maxsize=100)  # Limit queue size
#         self.is_streaming = False
#         self.stream_callback_active = False
...
#         if self.midi_in:
#             self.stop_capture()
#             self.stop_streaming()  # NEW: Stop streaming too
...
#         """Handle incoming MIDI messages - BOTH for capture AND streaming."""
#         if not message:
#             return
...
#         # EXISTING CODE: Handle batch capture (unchanged)
...
#         # NEW CODE: Handle real-time streaming
#         if self.is_streaming and self.stream_callback_active:
#             try:
#                 # Put message in queue for WebSocket streaming
...
#         """Start real-time MIDI streaming (separate from batch capture)"""
#         if not self.midi_in:
#             print("❌ No MIDI device connected for streaming")
#             return False
            
#         if self.is_streaming:
#             print("⚠️  MIDI streaming already active")
#             return True
        
#         self.is_streaming = True
#         self.stream_callback_active = True
...
#         print("🎹 Real-time MIDI streaming started")
#         return True
...
#         print("🎹 Real-time MIDI streaming stopped")

#     def get_stream_message(self):
#         """Get next MIDI message from stream (non-blocking)"""
#         try:
#             return self.streaming_queue.get_nowait()
#         except queue.Empty:
#             return None
    
#     # EXISTING CAPTURE METHODS (unchanged)
...
#             "is_streaming": self.is_streaming,  # NEW: Include streaming status
#             "stream_queue_size": self.streaming_queue.qsize()  # NEW: Queue size
#         }
