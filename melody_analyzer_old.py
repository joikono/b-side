"""
=============================================================================
MELODY TO CHORD PROGRESSION INFERENCE - THREE FOCUSED OPTIONS
=============================================================================

OVERVIEW:
This module analyzes melodic MIDI files to suggest THREE different chord progression
options, focusing on practical harmonization approaches.

KEY FEATURES:
• 🎼 Automatic key detection using Krumhansl-Schmuckler algorithm
• 🎵 THREE focused harmonization styles: Simple/Pop, Bass Foundation, Phrase Foundation
...
for root in range(12):
    ...
    JAZZ_CHORDS[root_name] = major_triad
    JAZZ_CHORDS[root_name + 'm'] = minor_triad
    JAZZ_CHORDS[root_name + '7'] = [(root + 0) % 12, (root + 4) % 12, (root + 7) % 12, (root + 10) % 12]
    JAZZ_CHORDS[root_name + 'm7'] = [(root + 0) % 12, (root + 3) % 12, (root + 7) % 12, (root + 10) % 12]
    JAZZ_CHORDS[root_name + 'maj7'] = [(root + 0) % 12, (root + 4) % 12, (root + 7) % 12, (root + 11) % 12]
    JAZZ_CHORDS[root_name + '6'] = [(root + 0) % 12, (root + 4) % 12, (root + 7) % 12, (root + 9) % 12]
    JAZZ_CHORDS[root_name + 'm6'] = [(root + 0) % 12, (root + 3) % 12, (root + 7) % 12, (root + 9) % 12]
...
def suggest_chord_jazz_style(segment_notes, key, scale_degrees):
    """Suggest chord using Jazz/Complex harmonization style."""
    chord_scores = {}
    pc_weights = defaultdict(float)
    ...
    for chord_name, chord_pcs in JAZZ_CHORDS.items():
        score = 0.0
        
        # More sophisticated chord tone analysis
        for pc, weight in pc_weights.items():
            if pc in chord_pcs:
                score += weight * 1.5
                if pc == chord_pcs[0]:  # Root
                    score += weight * 0.5
                elif len(chord_pcs) > 3 and pc == chord_pcs[3]:  # Extension (7th, 6th)
                    score += weight * 1.2  # Favor extended harmonies
            else:
                if pc in scale_degrees:
                    score -= weight * 0.02  # Very tolerant of scale tones
                else:
                    score -= weight * 0.2
        
        # Favor extended chords (7ths, maj7s, 6ths)
        if any(ext in chord_name for ext in ['7', '6', 'maj7']):
            score += 1.0
        
        # Bonus for jazz-style chord substitutions and extensions
        if 'maj7' in chord_name or '6' in chord_name:
            score += 0.5
...
def analyze_melody_three_ways(midi_file, segment_size=2, tolerance_beats=0.15):
    """
    ...
    # Analyze with three different styles
    simple_progression = []
    ...
    for seg_idx in range((max_beat + segment_size - 1) // segment_size):
        ...
        # Get chord suggestions from simple style
    ...
    create_three_foundation_visualization(midi_file, all_segments, bass_progression, phrase_progression, key, notes, "melody_three_foundations.png")
    
    # Output results
    print(f"\n🎵 Suggested Chord Progressions:")
    print(f"\nOption 1 (Simple/Pop): {' → '.join(simple_progression)}")
    print(f"Option 2 (Bass Foundation): {' → '.join(bass_progression)}")
    print(f"Option 3 (Phrase Foundation): {' → '.join(phrase_progression)}")
    print(f"\nConfidence scores: {simple_avg_conf:.2f}, {bass_conf:.2f}, {phrase_conf:.2f}")
    
    return key, (simple_progression, bass_progression, phrase_progression), (simple_avg_conf, bass_conf, phrase_conf), all_segments

def create_three_foundation_visualization(midi_file, segments, bass_progression, phrase_progression, key, notes, output_file):
    """Create visualization showing simple chords plus two foundation options."""
...
def main():
    ...
    print("=== MELODY TO CHORD PROGRESSION INFERENCE - THREE FOCUSED OPTIONS ===")
    key, progressions, confidences, segments = analyze_melody_three_ways(
        midi_file,
        segment_size=2,
    ...
    if key and progressions:
        simple_prog, bass_prog, phrase_prog = progressions
        simple_conf, bass_conf, phrase_conf = confidences
        ...
        print(f"🎵 Option 2 (Bass Foundation): {' → '.join(bass_prog)}")  
        print(f"🎵 Option 3 (Phrase Foundation): {' → '.join(phrase_prog)}")
        print(f"\n📊 Confidence scores: {simple_conf:.2f}, {bass_conf:.2f}, {phrase_conf:.2f}")
        print(f"="*60)
    else:
